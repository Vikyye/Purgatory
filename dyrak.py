...
class Player:
    ...
    def choose_attack_card(self, game):
        ...
        if not hand:
            return None
        if game.table:
            ranks_on_table = [c.rank for c, _ in game.table]
            possible = [c for c in hand if c.rank in ranks_on_table]
            if possible:
...
class Game:
    ...
    def update_animation(self):
        ...
        if self.table_animations and all_finished:
            ...
            if not self.deal_animation and self.turn == 'attack':
                self.waiting_for_action = True
                self.last_action_time = pygame.time.get_ticks()
                self.message = f"Ход {self.attacker.name}"
    ...
    def finish_attack(self):
        ...
        self.refill_delay = pygame.time.get_ticks() + 300
    ...
    def update(self):
        ...
        # ---- УНИВЕРСАЛЬНЫЙ ТАЙМЕР БЕЗДЕЙСТВИЯ (для игрока 10 сек, для бота 5 сек) ----
        if self.waiting_for_action and not self.game_over:
            if (self.turn == 'attack' and self.attacker.is_bot) or (self.turn == 'defense' and self.defender.is_bot):
                timeout = 5000
            else:
                timeout = 10000
            now = pygame.time.get_ticks()
            if now - self.last_action_time > timeout:
                if self.turn == 'attack':
                    self.finish_attack()
                    self.message = "Автоматическая бита (таймаут)."
                else:
                    self.finish_attack()
                    self.message = "Автоматическое завершение (таймаут)."
...
def draw_hud(game):
    ...
    pygame.draw.line(screen, GOLD, (0, 80), (WIDTH, 80), 2)

    name_text = font_hud_title.render(game.player.name, True, WHITE)
    screen.blit(name_text, (20, 10))
    level_text = font_hud.render("Уровень", True, WHITE)
    screen.blit(level_text, (20, 35))
    level_val = font_hud_value.render(str(game.level), True, GOLD)
    screen.blit(level_val, (20, 55))

    player_cards = len(game.player.hand)
    bot_cards = len(game.bot.hand)
    player_cards_text = font_hud.render("Карты: " + str(player_cards), True, WHITE)
    screen.blit(player_cards_text, (200, 10))
    bot_cards_text = font_hud.render("Бот: " + str(bot_cards), True, WHITE)
    screen.blit(bot_cards_text, (200, 35))

    coin_text = font_hud_title.render("Монеты", True, WHITE)
    screen.blit(coin_text, (400, 10))
    coin_val = font_hud_value.render(str(game.coins), True, GOLD)
    screen.blit(coin_val, (400, 35))

    score_text = font_hud.render("Победы:", True, WHITE)
    screen.blit(score_text, (600, 10))
    score_vals = font_hud_value.render(f"{game.player_score} - {game.bot_score}", True, GOLD)
    screen.blit(score_vals, (600, 35))

    extra1 = font_hud.render("5K", True, GOLD)
    screen.blit(extra1, (800, 10))
    extra2 = font_hud.render("12", True, WHITE)
    screen.blit(extra2, (800, 30))
    extra3 = font_hud.render("9", True, WHITE)
    screen.blit(extra3, (850, 10))
    extra4 = font_hud.render("10", True, WHITE)
    screen.blit(extra4, (850, 30))
    extra5 = font_hud.render("20", True, WHITE)
    screen.blit(extra5, (900, 10))
    extra6 = font_hud.render("25", True, WHITE)
    screen.blit(extra6, (900, 30))
    extra7 = font_hud.render("7.27K", True, GOLD)
    screen.blit(extra7, (950, 10))
...
def draw_game(game, pause_btn_rect, pause_btn_surf, paused):
    ...
    # ---- ТАЙМЕР (с учётом разных лимитов) ----
    if game.waiting_for_action and not game.game_over and not paused:
        if (game.turn == 'attack' and game.attacker.is_bot) or (game.turn == 'defense' and game.defender.is_bot):
            timeout = 5000
        else:
            timeout = 10000
        now = pygame.time.get_ticks()
        elapsed = (now - game.last_action_time) / 1000
        remaining = max(0, timeout/1000 - elapsed)
        timer_text = font_hud.render(f"{remaining:.1f}s", True, WHITE)
        screen.blit(timer_text, (WIDTH - 110, 55))
